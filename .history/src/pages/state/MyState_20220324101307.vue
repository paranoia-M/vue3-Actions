 ...
 * 改变state
 *    store.$patch({}),这样修改的话在遇到任何集合的修改(例如从数组中新增，删除，修改元素)都需要船舰一个新的集合，因此$patch也可以接受一个函数来批量修改集合内部对象的属性。(总结，上面的方法适合修改将基本数据类型)那么当组件被卸载时，他们将自动删除，如果要保留
 * 改变state的对象类型
 *    store.$patch((state) => {})
 * 替换state
...
store.$subscribe((mutation, state) => {
  ...
  console.log(333388888888999999, state);
});
