 ...
 * @LastEditTime: 2022-03-10 13:55:54
 ...
 *    vue2:卸载组件实例后调用。调用此钩子时，组件实例的所有指令都被解除绑定，所有事件侦听器都被移除，所有子组件实例被卸载。
 * 9. errorCaptured --> onErrorCaptured
 *    在捕获一个来自子组件的错误时被调用，此钩子会收到三个参数：错误对象、发送错误的组件实例以及一个包含错误来源信息的字符串。此钩子可以返回false以阻止该错误继续向上传播。
 * 10. activated --> onActivated
 *    vue2:被 keep-alive 缓存的组件激活时调用。
 * 11. deactivated --> onDeactivated
 *    vue2:被 keep-alive 缓存的组件停用时调用。
 * -------
 * 12. onRenderTracked
 *    跟踪虚拟dom，重新渲染时调用，钩子接收 debugger event 作为参数。页面上绑定了响应式数据（取值），就会触发该操作
 * 13. onRenderTriggered
 *     此事件告诉你是什么操作触发了重新渲染，以及该操作的目标对象和键。
 * 
 * 
 * 页面初始化,直接触发
 * beforeCreate，created，beforeMount，renderTracked，mounted
 * data不可用 --> data可用dom不可用 --> render函数首次被调用 --> 页面有取值操作时(绑定数据，插值语法)触发 --> 页面挂载完毕后触发
 * 
 * 数据发生改变后触发
 * renderTriggered,beforeUpdate,renderTracked,update
 * 改变数据 --> 数据发生改变后，dom更新之前 --> 页面有取值操作时(绑定数据，插值语法)触发 --> dom更新完毕后调用，update不会保证所有子组件也都被重新渲染完毕
 * 
 * 组件卸载
...
onRenderTracked(({ type, key, target, effect }) => {
...
// 第二阶段
onRenderTriggered(({ type, key, target, effect }) => {
  console.log({ type, key, target, effect });
})
onBeforeUpdate(() => {
...
onRenderTracked(({ type, key, target, effect }) => {
  /**
   * type: 操作类型，有get，has，iterate也就是取值操作
   * key:键，操作数据的key
   * target:响应式对象
   * effect:数据类型为function，作用为重新渲染
   * 
   */
  console.log('生命周期onRenderTracked');
  console.log({ key, target, type, effect });
})
